# Next Job Processing Workflow

**This document defines the complete workflow for processing the next highest-scoring job from the pipeline, including research, evaluation, and decision-making.**
...
**CRITICAL: Always read the job search criteria and requirements before analysis.**

Evaluate the job against the requirements documented in `JOB_REQUIREMENTS.md` and `JOB_SEARCH_CRITERIA.md`. Flag any concerns based on those documented requirements.

### Step 2.4: Early Checkpoint - Initial Assessment

**CRITICAL: STOP HERE and use AskUserQuestion tool for user decision.**

After displaying the job details and presenting factual observations, use the AskUserQuestion tool with these options:

**Use AskUserQuestion tool:**

```
Question: "Based on the job details above, how would you like to proceed?"
Header: "Next Action"
Options:
1. label: "Research company", description: "Continue to Step 3 - Research company culture, reviews, and management"
2. label: "Skip job", description: "Mark as reviewed and find next job in pipeline"
3. label: "Apply directly", description: "Skip research and proceed to resume generation"
```

**Action based on selection:**
-   **"Research company"** - Proceed to Step 3 (Company Research)
-   **"Skip job"** - Go to Step 5B (mark reviewed, find next job)
-   **"Apply directly"** - Skip to Step 5A (import and generate resume)

**Common reasons to skip at this stage:**
-   Niche technical requirements not in skillset
-   Required onsite days
-   Primary tech stack completely mismatched
-   Any automatic disqualifiers from JOB_REQUIREMENTS.md
...
**Use AskUserQuestion tool after presenting company research:**

```
Question: "Based on the company research above, how would you like to proceed with [Company Name]?"
Header: "Decision"
Options:
1. label: "Proceed with application", description: "Import job and generate customized resume and cover letter"
2. label: "Skip this job", description: "Mark as reviewed and move to next job in pipeline"
3. label: "Research more", description: "Gather additional company information before deciding"
```

**Action based on selection:**
-   **"Proceed with application"** - Go to Step 5A (import job and generate resume)
-   **"Skip this job"** - Go to Step 5B (mark reviewed, find next job)
-   **"Research more"** - Perform additional web searches or specific research as requested
